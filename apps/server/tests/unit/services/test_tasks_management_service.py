"""Unit tests for TasksManagementService event-driven routing."""
...
from task_orchestrator.domain.scoped_task import ScheduledScopedTask, StartedScopedTask, SuccessfullyFinishedScopedTask
...
# --- event-driven schedule dispatches the ready frontier ---
...
# --- dispatch successors on success and skip ---
...
# --- failure dispatches nothing; exactly-once fan-in ---
...
def test_root_task_not_dispatched_as_successor(jobs_repo, broker):
    ...
    spec_a = make_spec(T.RELOAD_PATIENT_DATA)
    spec_root = make_spec(T.RELOAD_SOMATIC_MUTATIONS)
    started_a = _make_started_task(spec_a)
