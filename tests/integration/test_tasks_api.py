...
def test_init_scope_creates_job(client: TestClient):
    scope_id = _scope_id()
    resp = client.post(f"/scopes/{scope_id}")
...
def test_get_tasks_returns_all_specs(client: TestClient):
    scope_id = _scope_id()
    client.post(f"/scopes/{scope_id}")
    resp = client.get(f"/scopes/{scope_id}/tasks")
...
def test_get_tasks_for_unknown_scope_returns_404(client: TestClient):
    resp = client.get(f"/scopes/{_scope_id()}/tasks")
...
def test_schedule_task_returns_202_and_pending_tasks(client: TestClient):
    ...
    client.post(f"/scopes/{scope_id}")
    resp = client.post(
        f"/scopes/{scope_id}/tasks/RELOAD_PATIENT_DATA/schedule",
    )
...
def test_schedule_unknown_task_returns_422(client: TestClient):
    scope_id = _scope_id()
    client.post(f"/scopes/{scope_id}")
    resp = client.post(f"/scopes/{scope_id}/tasks/NOT_A_REAL_TASK/schedule")
...
def test_abort_unknown_launch_returns_404(client: TestClient):
    scope_id = _scope_id()
    client.post(f"/scopes/{scope_id}")
    client.post(f"/scopes/{scope_id}/tasks/RELOAD_PATIENT_DATA/schedule")
    resp = client.delete(f"/scopes/{scope_id}/tasks/RELOAD_PATIENT_DATA/launches/{uuid.uuid4()}")
    assert resp.status_code == 404
